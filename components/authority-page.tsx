...
export function AuthorityPageView({ page }: { page: AuthorityPage }) {
  ...
  return <>
    ...
    <section className="bg-slate-50 py-20"><div className="container grid gap-10 lg:grid-cols-[.7fr_1.3fr]"><div><p className="eyebrow">Approfondisci</p><h2 className="mt-4 text-3xl font-semibold tracking-[-.04em] text-navy">Collega questa competenza al quadro d’insieme.</h2></div><div className="grid gap-3 sm:grid-cols-3"><Related href={`/servizi/${service.slug}`} label="Servizio" title={service.title} /><Related href={`/settori/${page.relatedSector}`} label="Settore" title="Applicazioni per settore" /><Related href={`/blog/${page.relatedArticle}`} label="Insight" title="Leggi l’approfondimento" /></div></div></section>
    <section className="container grid gap-10 py-20 sm:py-28 lg:grid-cols-[.7fr_1.3fr]"><div><p className="eyebrow">Domande frequenti</p><h2 className="mt-4 text-3xl font-semibold tracking-[-.04em] text-navy">Chiarimenti essenziali.</h2></div><div className="divide-y divide-slate-200">{page.faqs.map((faq) => <details className="group py-5" key={faq.question}><summary className="cursor-pointer list-none font-semibold text-navy focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-gold">{faq.question}</summary><p className="mt-3 leading-7 text-slate-600">{faq.answer}</p></details>)}</div></section>
    <CtaBanner title={`Portiamo chiarezza nel tuo progetto di ${page.title.toLowerCase()}.`} description="Un primo confronto serve a definire obiettivi, dati disponibili e prossimi passi concreti." />
  </>;
}

function Related({ href, label, title }: { href: string; label: string; title: string }) { return <Link href={href} className="group flex min-h-40 flex-col justify-between border border-slate-200 bg-white p-5 text-navy transition hover:border-gold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gold"><span className="eyebrow">{label}</span><span className="flex items-end justify-between gap-3 font-semibold">{title}<ArrowUpRight className="size-4 shrink-0 transition group-hover:translate-x-1 group-hover:-translate-y-1" /></span></Link>; }
